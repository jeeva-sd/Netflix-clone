...
import { addList, removeList } from '~/state/reducers/watchlist';
...
export default function MovieList() {
    ...
    const filteredMovies = trendingMovieData.filter((movie: any) =>
                ...
                {filteredMovies.map((movie) => (
                    <div key={movie.id} className='lg:w-2/12 w-6/12 border flex justify-center items-end flex-wrap'>
                        ...
                        <button
                            ...
                            className={`text-center ${Watchlist.includes(movie.id) ? 'bg-red-600' : 'bg-green-600'} p-2 w-full h-10`}>
                            {Watchlist.includes(movie.id) ? 'Added to Watchlist' : 'Add to Watchlist'}
                        </button>
                    </div>
                ))}
